...
const TeacherDatabase = () => {
  ...
  const columns = [
    ...
    //     <Link
    //       className="btn btn-sm btn-success"
    //       to={`/techpaysliposms?details=${JSON.stringify(row)}`}
    //     >
    //       Payslip OSMS
    //     </Link>
    //   ),
    ...
    <div className="container-fluid text-center my-3">
      ...
      {showTable && !showDelTeachers ? (
        <>
          ...
          <Link className="btn btn-sm btn-info m-3" to={`/JulySalary`}>
            July Salary Data
          </Link>
          <Link className="btn btn-sm btn-success m-3" to="/AddTeacher">
            Add Teacher
          </Link>
          <button
            type="button"
          ...
          subHeaderComponent={
            <input
              ...
              onChange={(e) => {
                ...
                setFilteredDelTeachers(
                  allDelTeachers.filter((el) =>
                    el.tname
                      .toLowerCase()
                      .includes(e.target.value.toLowerCase())
                  )
